...
test('Filter 404', async ({ page }) => {
  ...
  const expectedMessage = `No se encontró ningún resultado con location = ${testLocation}`;
  ...
  // Comprobar que no hay datos al filtrar
  //const dataAfterFilter = page.locator('table tbody tr').filter({ has: page.locator('td') });

  //await expect(dataAfterFilter).toHaveCount(0);
